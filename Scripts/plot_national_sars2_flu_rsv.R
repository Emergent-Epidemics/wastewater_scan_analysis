...
#Data#
######
sars2_dat <- read.csv("../Data/WWSCAN_SARS-CoV-2_N_Gene_-_all_variants_Multiple_Locations_20230310.csv")

sars2_dat$collection_date <- as.POSIXct(strptime(sars2_dat$collection_date, format = "%Y-%m-%d"))

rsv_dat <- read.csv("../Data/WWSCAN_Respiratory_syncytial_virus_(RSV)_RSV_Multiple_Locations_20230310.csv")

rsv_dat$collection_date <- as.POSIXct(strptime(rsv_dat$collection_date, format = "%Y-%m-%d"))

flu_dat <- read.csv("../Data/WWSCAN_Influenza_Influenza_A_Multiple_Locations_20230310.csv")
...
flu_dat$pop_norm <- as.numeric(flu_dat$Influenza_A_gc_g_dry_weight_pmmov)/flu_dat$sewershed_pop
